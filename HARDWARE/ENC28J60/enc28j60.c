...
static void ENC28J60_SPI2_Init(void)
{
	...
	RCC_APB1PeriphClockCmd(	RCC_APB1Periph_SPI2,  ENABLE );//SPI2时钟使能 	
   	RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD, ENABLE );//PORTB,D,G时钟使能 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;//PG6/7/8 推挽  上拉	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz	
 	GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化//PG6/7/8
 	GPIO_SetBits(GPIOC,GPIO_Pin_7);//PG6/7/8上拉	
...
{		
	...
	while(!(ENC28J60_Read(ESTAT)&ESTAT_CLKRDY)&&retry<5000)//等待时钟稳定
	{
		retry++;
		delay_ms(1);
	};
	if(retry>=5000)return 1;//ENC28J60初始化失败
	// do bank 0 stuff
